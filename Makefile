...
.PHONY: all deps-go deps-js deps build-go build-server build-cli build-js build build-docker-full build-docker-full-ubuntu lint-go golangci-lint test-go test-js gen-ts test run run-frontend clean devenv devenv-down protobuf drone help dist build_from_dist push_from_dist
...
BASE_COMMIT=d915e734052ab3e27f3677ae85a35d587134fbab
dist:
	rm -rf grafana-dist
