...
.PHONY: all deps-go deps-js deps build-go build-server build-cli build-js build build-docker-full build-docker-full-ubuntu lint-go golangci-lint test-go test-js gen-ts test run run-frontend clean devenv devenv-down protobuf drone help gen-go gen-cue dist build_from_dist push_from_dist
...
BASE_COMMIT=64017e8ca6825b79a2acb887a1c1d9088e47ed72
dist:
	rm -rf grafana-dist
...
build_from_dist: dist
	./build.sh public/build/static/source_$(shell git rev-parse HEAD).tar.gz $(shell git rev-parse HEAD)

push_from_dist: build_from_dist
	docker push ghcr.io/goldmansachs/grafana:9-ubi8-$(shell git rev-parse HEAD)
